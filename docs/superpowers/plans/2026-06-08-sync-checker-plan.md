...
**Goal:** Create a local CLI script (`npm run check-sync`) that checks and reports git deltas between the fork's current branch and the upstream production-tracking branch (default `upstream/master`).

**Architecture:** A standalone TypeScript script run via `tsx` (existing devDependency) that executes local `git` sub-processes (`git fetch`, `git remote`, `git log`, `git diff`) and outputs structured, colorized terminal reports indicating commits behind, commits ahead, and modified files.

**Tech Stack:** Node.js, TypeScript, TSX, Git CLI.

---

### Task 1: Create the check-sync script

**Files:**
- Create: `scripts/check-sync-delta.ts`

- [ ] **Step 1: Write the sync checker script**

Create `scripts/check-sync-delta.ts` with the following implementation:
...
In `package.json`, locate the `"scripts"` field and add:
`"check-sync": "tsx scripts/check-sync-delta.ts"`

- [ ] **Step 2: Test script run with default options**

Run: `yarn check-sync` (or `npm run check-sync`)
Expected:
1. Fetches from upstream.
2. Reports the delta (since master is clean, it should show any differences between our branch and `upstream/master`).

- [ ] **Step 3: Test script run with custom branch override**

Run: `yarn check-sync --branch develop`
Expected:
1. Compares current branch against `upstream/develop`.
2. Prints commits behind `upstream/develop` and changed files.

- [ ] **Step 4: Test offline/no-fetch mode**

Run: `yarn check-sync --no-fetch`
Expected:
1. Instantly runs and compares without the fetching line.

- [ ] **Step 5: Commit changes**

Add `scripts/check-sync-delta.ts` and `package.json`.
Commit with message `"feat: add check-sync command line tool"`
